...
- Dropout regularisation was not used in this case as the autoencoder is already extracting compact and consise features into the bottlenack, so deactivating random neurons would make the training process more difficult and make the model learn random pathways.
- Scheduling is also not used here as there is not much skew in the loss values as we go through the epochs.
- Instead of dropout, denoising has been used with a factor of 0.05 to inject noise into the images during training to directly impact the input data, followed by comparison with the original images so that the model is forced to learn the actual features instead of memorising the pixel values.
